            ...
            db.collection("absences").where("eleve", "==", eleve.id)
                ...
                <div class="card mb-2  pl-5 border-left-danger" id='${absence.id}'>
                            ...
                            <span class="text">Justifier</span>
                              ...
                              <span class=" mt-2 float-right">${absence.data().date} - ${absence.data().hr_debut} à ${absence.data().retard == true ? absence.data().hr_fin : getRetard(absence)[0] + ':' + getRetard(absence)[1]} </span>
                            </div>
                          </div>
                `)
                        }
...
let timer = 0
db.collection('eleves').doc(localStorage.getItem('ID')).get().then((eleve) => {
    db.collection('classes').doc(eleve.data().classe).onSnapshot((classse) => {
        $('#statut_cours').empty()
